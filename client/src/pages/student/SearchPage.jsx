...
const SearchPage = () => {
  ...
  // treat missing query as empty string so API returns all published courses
  ...
  // debug helpers – log API results to console for troubleshooting
  React.useEffect(() => {
    if (!isLoading) {
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      <div className="my-6">
        <h1 className="font-bold text-xl md:text-2xl">
          {query ? `result for "${query}"` : "All Courses"}
        </h1>
        {query && (
          <p>
            Showing results for{""}
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-32 dark:bg-gray-900 p-6">
      ...
      <h1 className="font-bold text-2xl md:text-4xl text-gray-800 dark:text-gray-200 mb-2">
        {query ? "Course Not Found" : "No Courses Available"}
      </h1>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-4">
        {query
          ? "Sorry, we couldn't find the course you're looking for."
          : "There are currently no published courses. Please check back later or contact an instructor."}
      </p>
